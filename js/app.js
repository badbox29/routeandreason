...
function populateSettingsModal() {
  ...
  if (syncIndicator) {
    if (Auth.isGuest()) {
      ...
      syncIndicator.style.cssText = 'border-color:rgba(74,124,89,.4);background:rgba(74,124,89,.12);color:var(--green-mid);';
    } else {
      syncIndicator.textContent = 'No worker URL';
      syncIndicator.style.cssText = 'border-color:var(--border);color:var(--ink-muted);';
    }
  }
...
document.getElementById('modal-settings').addEventListener('click', e => {
  ...
  if (e.target.closest('#btn-manual-sync')) {
    ...
    if (indicator) { indicator.textContent = 'Syncing…'; indicator.style.cssText = 'border-color:var(--border);color:var(--ink-muted);'; }
    saveProfileToKV()
      .then(() => {
        if (indicator) { indicator.textContent = '✓ Synced'; indicator.style.cssText = 'border-color:rgba(74,124,89,.4);background:rgba(74,124,89,.12);color:var(--green-mid);'; }
        showToast('Synced ✓');
      })
      .catch(() => {
        if (indicator) { indicator.textContent = 'Sync failed'; indicator.style.cssText = 'border-color:rgba(192,57,43,.4);background:rgba(192,57,43,.08);color:var(--red-soft);'; }
